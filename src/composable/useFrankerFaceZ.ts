...
function createOverrides(): void {
	...
	if (profile) {
		...
		settings.main_context.updateContext({
			[`profile.${profile.id}.enabled`]: true,
		});
	} else {
		if (typeof settings.createProfile !== "function") return;
		profile = settings.createProfile({
			name: "7TV",
			description: "This profile was generated by 7TV and modifies some settings for compatibility",
			context: [
				{
					"disable-chat-processing": true,
				},
			],
			hotkey: null,
			pause_updates: null,
