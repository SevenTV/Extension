...
const data = reactive({
	...
	profile: {
		...
		persisted: null as FFZSettingsProfile | null,
	},
});
...
function createOverrides(): void {
	...
	let persistedProfile = settings.__profiles.find((p) => p.name === "7TV Persistent");
	let profile = settings.__profiles.find((p) => p.name === "7TV Ephemeral");
	...
	if (!persistedProfile) {
		persistedProfile = data.profile.persisted = settings.createProfile({
			name: "7TV Persistent",
			description:
				"This profile was generated by 7TV and permanently modified some settings for compatibility. If you've disabled 7TV, you can safely delete this profile to restore settings.",
			context: [],
			hotkey: null,
			pause_updates: null,
			toggled: true,
	...
	// Set up persisting config toggles
	//
	// in some cases FFZ settings need a refresh to take effect, so we need to
	// persist these settings to a profile that is always enabled
	if (persistedProfile) {
		persistedProfile.set("chat.inline-preview.enabled", false);
	}

	// Move profile to top
	if (typeof settings.moveProfile === "function") {
		settings.moveProfile(profile.id, 0);
		if (persistedProfile) settings.moveProfile(persistedProfile.id, 1);
	}
